...
public abstract class HttpSapBusinessFactory extends HttpBusinessFactory {
     ...
     * and update exist in two modes (Post and Put)
     */
     ...
     * UrlMandatory, UrlOptional, HeaderMandatory, HeaderOptional,BodyMandatory,
     * BodyOptional, SecurityOptional. If one SapArg is in the array, it has the
     * status associated with the rank. One SapArg can appears in multiple rank.
     */
	...
	public static HttpPageHandler initializeHttpPageHandler() {
		...
		try {
			...
			if (!HttpBusinessFactory.addDefaultErrorPages(pageHandler, "SAP ERROR", HttpSapBusinessFactory.class)) {
			...
			pages.put(uri, new HttpPage(pagename, null, header, footer, beginform, endform, nextinform, 
	...
	private static void addDefaultFields(LinkedHashMap<String, AbstractHttpField> linkedHashMap,
		...
		for (int j = 0; j < fields[rank].length; j++) {
			...
			linkedHashMap.put(fieldname, new DefaultHttpField(fieldname, fieldRole, fieldinfo, fieldvalue, 
					fieldvisibility, fieldmandatory, fieldcookieset, true, FieldPosition.URL, fieldrank));
		}
		rank++;
		for (int j = 0; j < fields[rank].length; j++) {
			SapField field = fields[rank][j];
		...
		for (int j = 0; j < fields[rank].length; j++) {
			...
			fieldrank = ++nb;
			linkedHashMap.put(fieldname, new DefaultHttpField(fieldname, fieldRole, fieldinfo, fieldvalue, 
					fieldvisibility, fieldmandatory, fieldcookieset, true, FieldPosition.HEADER, fieldrank));
		}
		rank++;
		for (int j = 0; j < fields[rank].length; j++) {
			SapField field = fields[rank][j];
		...
		for (int j = 0; j < fields[rank].length; j++) {
			...
			fieldrank = ++nb;
			linkedHashMap.put(fieldname, new DefaultHttpField(fieldname, fieldRole, fieldinfo, fieldvalue, 
					fieldvisibility, fieldmandatory, fieldcookieset, true, FieldPosition.BODY, fieldrank));
		}
		rank++;
		for (int j = 0; j < fields[rank].length; j++) {
			SapField field = fields[rank][j];
		...
		for (int j = 0; j < fields[rank].length; j++) {
			...
			fieldrank = ++nb;
			linkedHashMap.put(fieldname, new DefaultHttpField(fieldname, fieldRole, fieldinfo, fieldvalue, 
					fieldvisibility, fieldmandatory, fieldcookieset, true, FieldPosition.ANY, fieldrank));
		}
	}
