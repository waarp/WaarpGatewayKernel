...
public abstract class HttpRequestHandler extends SimpleChannelUpstreamHandler {
        ...
        if (!cookies.isEmpty()) {
            for (Cookie cookie: cookies) {
            	if (isCookieValid(cookie)) {
            		httpPage.setValue(businessRequest, cookie.getName(), cookie.getValue(), FieldPosition.COOKIE);
            	}
            }
        }
        cookies.clear();
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) {
    	...
    	try {
	        if (!readingChunks) {
				...
				WaarpActionLogger.logCreate(DbConstant.admin.session, "Request received: "+httpPage.pagename, session);
     ...
     * @throws HttpIncorrectRequestException 
     */
    protected void prepareError(Channel channel, String message) throws HttpIncorrectRequestException {
    ...
    protected void writeSimplePage(Channel channel) throws HttpIncorrectRequestException {
    	logger.debug("HttpPage: "+(httpPage != null ? httpPage.pagename : "no page")+
    			" businessRequest: "+(businessRequest != null ? businessRequest.getClass().getName() : "no BR"));
    	if (httpPage.pagerole == PageRole.ERROR) {
    		try {
    			httpPage.setValue(businessRequest, AbstractHttpField.ERRORINFO, errorMesg, FieldPosition.BODY);
    		} catch (HttpIncorrectRequestException e) {
    			// ignore
    		}
    	}
     ...
     * (this method must send back the answer using for instance a ChunkedInput handler and should try to
     * call clean(), but taking into consideration that it will erase all data, so it must be ensured that
     * all data are sent through the wire before calling it. Note however that when the connection is
     * closed or when a new request on the same connection occurs, the clean method is automatically called.
     * The usage of a HttpCleanChannelFutureListener on the last write might be useful.)
     * @param channel
     */
     ...
     * @throws HttpIncorrectRequestException
     */
    public abstract void businessValidRequestAfterAllDataReceived(Channel channel) throws HttpIncorrectRequestException;
    ...
    protected boolean getBodyAsFile(MessageEvent e) throws HttpIncorrectRequestException {
    	...
    	if (fileuploadField.fieldtovalidate) {
			if (! businessRequest.isFieldValid(fileuploadField)) {
				throw new HttpIncorrectRequestException("Field unvalid: "+fileuploadField.fieldname);
			}
		}
    	return true;
    }
    ...
    /**
     * Read request by chunk and getting values from chunk to
     * chunk
     * 
     * @param channel
     * @throws HttpIncorrectRequestException 
     ...
     * @throws HttpIncorrectRequestException 
     */
    protected void readHttpData(InterfaceHttpData data, Channel channel) throws HttpIncorrectRequestException {
