...
public abstract class DataModelRestMethodHandler<E extends AbstractDbData> extends RestMethodHandler {
    ...
    /**
     * allowed: GET iff name or name/id, PUT iff name/id, POST iff name (no id), DELETE iff name/id
     * and allowed
     */
    @Override
    ...
    /**
     * For Read or Update, should include a select() from the database. Shall not be used for
     * Create. JSON_ID should be checked for the primary id.
     * 
     * @param handler
     * @param arguments
     * @param result
